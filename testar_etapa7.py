"""
testar_etapa7.py — Entrypoint pra testar Etapas 1 a 7 em sequência.
...
Rode com:
    streamlit run testar_etapa7.py
"""
...
st.set_page_config(page_title="Etapas 1-7 — Teste", layout="wide")
st.title("Analisador de Propostas — Etapas 1 a 7")
st.caption(
...
st.progress(passo / 9, text=f"Passo {passo} de 9")
...
# -----------------------------------------------------------------------
if st.session_state.etapa7_resultado:
    st.success("Etapas 1 a 7 concluídas.")
    ...
    with col_b:
        if st.session_state.etapa7_resultado.get("tem_dados"):
