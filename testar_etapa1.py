...
if st.session_state.etapa1_resultado and not st.session_state.etapa1_confirmada:
    ...
    if entrada:
        ...
        if any(p in entrada_lower for p in palavras_confirmacao):
            ...
            resposta = (
                "✅ **Etapa 1 confirmada.**\n\n"
                f"- **Cliente:** {estudo.cliente or '—'}\n"
                f"- **Categoria:** {estudo.categoria}\n"
                f"- **Modelo de precificação:** {estudo.modelo_precificacao}\n"
                f"- **Proponentes:** {len(estudo.proponentes)}\n"
                f"- **Cenários de comparação:** {1 + len(estudo.proponentes)} "
                f"(As Is + {len(estudo.proponentes)} proponentes)\n\n"
    ...
    with st.expander("Ver estado do Estudo após Etapa 1"):
        st.write("**Cliente:**", estudo.cliente or "—")
        ...
        st.write("**Proponentes (agrupamento por fornecedor):**")
        if estudo.proponentes:
            for i, p in enumerate(estudo.proponentes, start=1):
                arq = p.get("arquivos", {})
                st.write(
                    f"- **P{i} — {p.get('fornecedor', '?')}** | "
                    f"técnica: {arq.get('tecnica') or '—'} · "
                    f"comercial: {arq.get('comercial') or '—'} · "
                    f"combinada: {arq.get('combinada') or '—'}"
                )
            n_cenarios = 1 + len(estudo.proponentes)
            st.write(f"**Cenários de comparação:** {n_cenarios} (As Is + {len(estudo.proponentes)} proponentes)")
        else:
            st.write("- _nenhum_")

        if estudo.faltantes:
            st.write("**Faltantes / atenção:**")
